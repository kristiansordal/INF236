...
#pragma omp barrier
    ...
    if (ABFS)
        store_p_value(wf, "TimeAParallelBFS", p_timer[1], n_graphs,
                      n_conf); // Print timing results for alt parallel BFS
...
int verify_bfs(int n, int *p1, int *p2) {
    ...
    // printf("There are %d errors in the distance values\n", errors);
    // printf("There are %d vertices that are not found in the parallel BFS\n", notfound);
    return (true);
}
