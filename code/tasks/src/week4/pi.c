...
//         //      srandom_r(id+1,rand_state);                 // Use this line to get the same random numbers for each
//         run srandom_r(time(NULL) + id + 1, rand_state); // Use this line to get new random numbers for each run
...
#pragma omp parallel
    {
        ...
        // Allocate memory for points within each thread to avoid false sharing
        struct Point *points = (struct Point *)malloc(sizeof(struct Point) * num_points / omp_get_num_threads());

        for (int i = 0; i < num_points / omp_get_num_threads(); i++) {
            struct Point p;
            p.x = (double)rand_r(&seed) / RAND_MAX;
            p.y = (double)rand_r(&seed) / RAND_MAX;
            points[i] = p;
