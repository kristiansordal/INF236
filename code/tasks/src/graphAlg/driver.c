    ...
    */
    ...
    for (i = 0; i < n_graphs; i++) {
        ...
        */
        ...
        // Compute an independent set (IS) using a greedy algorithm
        // Upon return p[v] will be true if and only v is in the IS
        ...
        if (IS) {
            ...
            printf("************************************************\n");
            printf("*  Sequential independent set                  *\n");
            printf("*  Time = %8.6lf seconds, size = %d    *\n", timer[0][i], count);
            printf("************************************************\n");
        }

        // Compute a graph coloring. In each step a vertex is colored with the lowest possible color
        // Upon return p[v] will contain the color of vertex v (numbered from 0 and upwards).
        ...
        if (GC) {
            for (l = 0; l < n_runs; l++) {
                ...
                mt2 = omp_get_wtime();                              // Get current time
                if ((timer[1][i] < 0) || (mt2 - mt1 < timer[1][i])) // Capture best run
                    timer[1][i] = mt2 - mt1;
            }
            ...
            printf("************************************************\n");
            printf("*  Sequential graph coloring                   *\n");
            printf("*  Time = %8.6lf seconds, number colors = %d   *\n", timer[1][i], count);
            printf("************************************************\n");
        }
...
#pragma omp barrier
                for (k = 0; k < n_runs; k++) { // Run all experiments n_runs times, select best run time as final
                    ...
                    if (PIS) { // Parallel IS, IS values are returned in p
#pragma omp barrier
#pragma omp master
                        { mt1 = omp_get_wtime(); }
                        pis(n[i], ver, edges, p, p3, p4);
...
#pragma omp master
                {
                    if (PIS) {
                        ...
                        printf("*  Parallel IS                                 *\n");
                        printf("*  Time = %8.6lf seconds, size = %d    *\n", p_timer[0][i][j], count);
                        printf("*  Speedup = %4.2lf, Relative speedup = %4.2lf     *\n", timer[0][i] / p_timer[0][i][j],
                               p_timer[0][i][0] / p_timer[0][i][j]);
                        printf("************************************************\n");
                    }
                } // master
...
#pragma omp barrier
                for (k = 0; k < n_runs; k++) { // Run all experiments n_runs times, select best run time as final
                    ...
                    if (PGC) { // Parallel Graph Coloring
#pragma omp barrier
#pragma omp master
                        { mt1 = omp_get_wtime(); }
                        pgc(n[i], ver, edges, p, p3, p4);
...
#pragma omp master
                {
                    if (PGC) {
                        ...
                        printf("*  Parallel GC                                 *\n");
                        printf("*  Time = %8.6lf seconds, number colors = %d   *\n", p_timer[1][i][j], count);
                        printf("*  Speedup = %4.2lf, Relative speedup = %4.2lf     *\n", timer[1][i] / p_timer[1][i][j],
                               p_timer[1][i][0] / p_timer[1][i][j]);
                        printf("************************************************\n");
                    }
                } // master
...
#pragma omp barrier
    ...
    // Sequential timings
    // ******************
    // timer[0][]	Sequential IS
    // timer[1][]	Sequential GC
    //
    // Parallel timings, all timings gives the best run
    // ****************
    // p_timer[0][i][conf]	Parallel IS
    // p_timer[1][i][conf]	Parallel GC
    //

    if (IS)
        store_value(wf, "TimeSequentialIS", timer[0], n_graphs); // Print timing results for sequential IS

    if (GC)
        store_value(wf, "TimeSequentialGC", timer[1], n_graphs); // Print timing results for sequential GC

    // Print out parallel values

    if (PIS)
        store_p_value(wf, "TimeParallelIS", p_timer[0], n_graphs, n_conf); // Print timing results for parallel IS

    if (PGC)
        store_p_value(wf, "TimeParallelGC", p_timer[1], n_graphs, n_conf); // Print timing results for alt parallel GC
...
int read_bin_graph(int *n, int *m, char *f_name, int **ver, int **edges, edge **e, double **weight, int *count,
                   int *start) {
...
int read_graph(int *n, int *m, char *f_name, int **ver, int **edges, edge **e, double **weight, int *count, int *start,
               int *where) {
...
int read_general_graph(int *n, int *m, char *f_name, int **ver, int **edges, edge **e, double **weight, int *count,
                       int *start) {
...
int allocate_memory(int size, int **n, int **m, double ***timer, double ***cost, double ****p_timer, int ****nrC,
                    int n_conf) {
...
int allocate_graph_memory(double **weight, edge **e, int **ver, int **edges, int n, int **p, int **p1, int **p2,
                          int **p3, int **p4) {
