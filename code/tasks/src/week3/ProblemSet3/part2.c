...
int main(int argc, char **argv) {
    ...
    /*
      ...
      this can be made parallel. Make sure that the printed output
      x is the same when running on one thread and when you use several threads.
    */
...
#pragma omp section
    ...
    */
    ...
    dtime = gettime();
#pragma omp parallel for private(scal, j, k) // which variables are local
    for (i = 0; i < MATSIZE; i++) {
        for (j = 0; j < MATSIZE; j++) {
            scal = 0.0;
            for (k = 0; k < MATSIZE; k++) {
    ...
    */
    ...
    for (i = 0; i < MATSIZE; i++) {
#pragma omp parallel for private(k) lastprivate(scal) // which variables are local
        for (j = 0; j < MATSIZE; j++) {
            scal = 0.0;
            for (k = 0; k < MATSIZE; k++) {
                scal += mat_a[i][k] * mat_b[k][j];
